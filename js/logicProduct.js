...
function botonDelCarritoDisabled(id) {
    const bottonOutStock = document.getElementById(`stock${id}`).innerHTML = "Sin Stock"
    const buttonCompraDisabled = document.getElementById(`btnCart${id}`).innerHTML = `<button
    onclick= agregar()
...
const agregar = (id) => {
    ...
    if (!productoSeleccionado) {
        ...
        let nImg = nProd.imagen;
        tuCarrito.push({ id: id, cant: 1, nombre: nNombre, precio: nPrecio, imagen: nImg })
    } else {
